way to deployment:

    cd into the dir
    run in terminal:
        sudo npm install
        nmp start
    use your nginx to listen 80 and forward it to 4600 port

file struct:

    bin:            used to setup the server.
    node_modules:   essential decpendency accesories.
    public:         static folders.
    routes:         used for floor-1 routers.
    views:          http templates.
    webentity:      website generator.
    weixin_basic:   wechat basic accessories. (API, etc.)
    weixin_handler: the handler for wechat official server.
    models:         the interface of databases

what to modify?

    To modify host ip:      address_configure.js
    To modify databases:    models/models.js
    To modify weixin keys:  weixin_basic/settings.js
    #REMEMBER TO ENSURE INDEX IN MONGODB, s'il vous plaît!

[![Coverage Status](https://coveralls.io/repos/cgcgbcbc/wxTicket_forTest/badge.svg)](https://coveralls.io/r/cgcgbcbc/wxTicket_forTest)
[![Build Status](https://travis-ci.org/cgcgbcbc/wxTicket_forTest.svg)](https://travis-ci.org/cgcgbcbc/wxTicket_forTest)